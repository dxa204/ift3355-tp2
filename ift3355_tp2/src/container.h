...
struct BVHNode {
    ...
    // Pointeur vers l'objet si c'est une feuille.  Sinon, nullptr.
    Object* object = nullptr;

    // Pour verifier si un noeud est une feuille.
    bool is_leaf() const {
...
private:
    ...
    BVHNode* recursive_build(std::vector<BVHObjectInfo> bvhs, int idx_start, int idx_end, int axis) {
        ...
        // If there is only one element, it is a leaf node. Stop recursion.
        if (idx_end - idx_start == 1) {
            node->left = node->right = nullptr;
            node->object = objects[bvhs[idx_start].idx]; // Set the object for the leaf node
            node->aabb = bvhs[idx_start].aabb;
        } else {
            // Internal node: proceed with recursive subdivision
            ...
            node->object = nullptr; // No object for internal nodes
        }
